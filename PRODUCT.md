...
- **Stage gates are load-bearing, but check the code, not just the checklist.** `CLAUDE.md`: "One stage at a time, in roadmap order. Nothing from a later stage leaks into an earlier one." Stage 3 (web app) is shipped; Stage 4 (admin/ops, private beta gate) is next. `README.md`'s checklist marked Stage 5 unstarted, but `backend/src/justnews_api/services/ranking.py` is a real, working heuristic ranker — recency × topic affinity × popularity × source trust × language match, MMR-diversified — already called by `/v1/feed` for any beta-access reader. It predates this session. Corrected in `README.md` once found (this session). Stage 6 (the FINDING model) is genuinely not started.
- **The ranker is built but unproven.** It's wired and live for anyone with beta access — but private beta hasn't opened (Stage 4's gate), so no real reader's feed has run through it yet, and ROADMAP.md's own Stage 5 done-criterion ("beats chronological on CTR among beta users, recorded in the experiment console") cannot be satisfied until Stage 4 opens real beta traffic. Built-ahead-of-activation, not evidence the stage gate was skipped.
- **Practical effect on onboarding:** anyone who reaches `/onboarding` already has beta access (`requireBetaAccess` gates the route) — so their language and topic picks feed the real ranker immediately, not a future one. Copy there can say so plainly.
- Personalization UI built for a reader with **no** beta access (the majority of any future private beta's early traffic, before invites land) still has nothing real to show — the anonymous/no-beta feed path (`getArticles`) never touches the ranker. That surface stays UI/UX groundwork against mock data, per the earlier decision for this work.
...
- No model inference in a request path (ADR 0004) — the heuristic ranker (Stage 5) is pure arithmetic over rows already fetched, not a forward pass, which is what ADR 0004 actually rules out; Stage 6's learned model is where that constraint will really be tested.
...
- An `/impeccable critique` run from this session (`frontend/.impeccable/critique/`) scored the current UI 26/40, since substantially addressed: i18n, pagination, the account menu's fake ARIA role, action-button feedback, mobile masthead, locale switcher path-preservation. A personalization-UX shape pass followed (research on Feedly/Inoreader/Artifact, `frontend/.impeccable/` and this file); shipped from it: dismissed cards gray out in place, a reading-language mix on Settings, and a "why am I seeing this?" component previewable at `/[locale]/dev/why` (blocked outside local development — see that file). Its reason data is still mock: `FeedItemOut` carries no reason field, so wiring it to the real ranker's actual scoring factors is the remaining step, now that the ranker itself is confirmed real rather than unbuilt. Real undo on "Not interested" is still blocked — needs a `DELETE` endpoint that doesn't exist.
